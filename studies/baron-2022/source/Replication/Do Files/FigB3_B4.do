...
DESCRIPTION:
This do-file generates FIGURES B3 AND B4, which summarize the distribution of 
referenda by month, as well as histograms and McCrary's tests for the vote-share
distributions. Note that this do-file requires the ADO file "DCdensity.do".

DATA INPUTS: (1) Referendum
OUTPUT: (1) FIG B3 AND FIG B4
*********************************/
...
*****************************************
******SECTION II: FIGURE B3 (DISTRIBUTION BY MONTH)
*****************************************
*During which month was the referendum held?
tab month
gen frequency=1

*Operational
graph hbar (sum) frequency if operational==1, over(month) lintensity(150) ytitle(Number of Questions) ///
bgcolor(white) graphregion(lcolor(white) ilcolor(white) fcolor(white) ifcolor(white)) ///
plotregion(lcolor(none) ilcolor(white) style(none)) ascategory blabel(bar) ///
yvar(relabel(1 "Jan." 2 "Feb." 3 "Mar." 4 "Apr." 5 "May" 6 "June" 7 "July" ///
8 "Aug." 9 "Sep." 10 "Oct." 11 "Nov." 12 "Dec."))
graph export "${path}\Output\monthdist_op.pdf", replace

*Capital
graph hbar (sum) frequency if operational==0, over(month) lintensity(150) ytitle(Number of Questions) ///
bgcolor(white) graphregion(lcolor(white) ilcolor(white) fcolor(white) ifcolor(white)) ///
plotregion(lcolor(none) ilcolor(white) style(none)) ascategory blabel(bar) ///
yvar(relabel(1 "Jan." 2 "Feb." 3 "Mar." 4 "Apr." 5 "May" 6 "June" 7 "July" ///
8 "Aug." 9 "Sep." 10 "Oct." 11 "Nov." 12 "Dec."))
graph export "${path}\Output\monthdist_cap.pdf", replace


*****************************************
******SECTION III: FIGURE B4 (HISTOGRAMS AND MCCRARY)
*****************************************
*Investigate whether there is any evidence of vote share manipulation 
*near the 50% threshold. If there is, this would violate identification assump.

*Operational
twoway histogram perc_yes if operational==1, fraction width(1) xline(50.2, lcolor(black)) ///
xtitle("Vote share in favor") bgcolor(white) graphregion(lcolor(white) ilcolor(white) ///
fcolor(white) ifcolor(white)) plotregion(lcolor(none) ilcolor(white) style(none)) 
graph export "${path}\Output\histogram.pdf", replace	

*Capital
twoway histogram perc_yes if bond==1, fraction width(1) xline(50.2, lcolor(black)) ///
xtitle("Vote share in favor") bgcolor(white) graphregion(lcolor(white) ilcolor(white) fcolor(white) ifcolor(white)) plotregion(lcolor(none) ilcolor(white) style(none)) 
graph export "${path}\Output\histogram2.pdf", replace	


*More formal McCrary's Test
cd "${path}\Do Files"

*Operational
DCdensity perc_yes if operational==1, b(1) breakpoint(50) generate(Xj Yj r0 fhat se_fhat)
gen upCI = fhat  + 1.96*se_fhat
gen downCI = fhat  - 1.96*se_fhat
graph twoway (scatter Yj Xj, mfcolor(white)) ///
(line fhat r0 if r0 <= 50, lcolor(black) lwidth(medthick)) ///
(line upCI r0 if r0 <= 50, lcolor(black) lpattern(dash)) ///
(line downCI r0 if r0 <=50, lcolor(black) lpattern(dash)) ///
(line fhat r0 if r0 > 50, lcolor(black) lwidth(medthick)) ///
(line upCI r0 if r0 > 50,  lcolor(black) lpattern(dash)) ///
(line downCI r0 if r0 >50, xline(50.2, lwidth(medthick)) lcolor(black) lpattern(dash) ///
xtitle("Vote share in favor") ytitle("Density")) ///
, legend(off) graphregion(color(white)) bgcolor(white) ylabel(0 "0" ///
0.01 ".01" 0.02 ".02" 0.03 ".03" 0.04 ".04" 0.05 ".05")
graph export "${path}\Output\histogram1.pdf", replace	
drop Yj Xj r0 fhat se_fhat upCI downCI

***Capital
DCdensity perc_yes if bond==1, b(1) breakpoint(50) generate(Xj Yj r0 fhat se_fhat)
gen upCI = fhat  + 1.96*se_fhat
gen downCI = fhat  - 1.96*se_fhat
graph twoway (scatter Yj Xj, mfcolor(white)) ///
(line fhat r0 if r0 < 50, lcolor(black) lwidth(medthick)) ///
...
(line fhat r0 if r0 > 50, lcolor(black) lwidth(medthick)) ///
(line upCI r0 if r0 > 50,  lcolor(black) lpattern(dash)) ///
(line downCI r0 if r0 >50, xline(50, lwidth(medthick)) lcolor(black) lpattern(dash) ///
xtitle("Vote share in favor") ytitle("Density")) ///
, legend(off) graphregion(color(white)) bgcolor(white) ylabel(0 "0" ///
0.01 ".01" 0.02 ".02" 0.03 ".03" 0.04 ".04" 0.05 ".05")
graph export "${path}\Output\histogram3.pdf", replace	
