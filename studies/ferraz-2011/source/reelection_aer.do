...
estout est1 using "../results/table4.csv", cells(`"b() se() t() p()"') stats(N r2 df_m df_r df_b df) notype abbrev begin(`"""') delimiter(`"",""') end(`"""') replace;
...
eststo: areg pcorrupt first first_p_cad_pref $prefchar2 $munichar2 lrec_trans lfunc_ativ p_cad_pref vereador_eleit ENLP2000 comarca sorteio* if esample2==1, robust abs(uf);
 *test first first_p_cad_pref;

eststo: areg pcorrupt first first_h_ENEP2000 h_ENEP2000 $prefchar2 $munichar2 lrec_trans p_cad_pref vereador_eleit comarca sorteio* if esample2==1, robust abs(uf);
 *test first first_h_ENEP2000;

estout using "../results/table10.csv", cells(`"b() se() t() p()"') stats(N r2 df_m df_r df_b df) notype abbrev begin(`"""') delimiter(`"",""') end(`"""') replace;
