...
merge 1:1 fips using "$outfile/lynchallstates";
drop _merge;
...
graph save "regpreact.gph", replace;
...
graph save "regpostact.gph", replace;

graph combine regpreact.gph regpostact.gph, xcommon ycommon graphregion(color(white));
graph export "$outfile2/proact.eps", as(eps) preview(off) replace;
