...
* Allocate the 86.6 billion dollars to states in proportion to their FY2007 medicaid expenditures.
bysort time: egen total_medicaid = total(medicaid_fy2007)
replace WSJ_health = 86600000000*medicaid_fy2007/total_medicaid if WSJ_health ~= .
drop total_medicaid
...
qui: desc WSJ*, varlist
foreach variable in `r(varlist)' {
	replace Amounts_wallstreet = Amounts_wallstreet + `variable' if `variable' ~= . 
	}
...
generate WSJ_percapita           = Amounts_wallstreet      / (StatePopulation*1000)
generate AmericanProgress_percap = AmericanProgressEstimates / (StatePopulation*1000)
