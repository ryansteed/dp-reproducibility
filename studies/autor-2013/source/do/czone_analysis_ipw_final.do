...
foreach z in sh_empl sh_empl_mfg sh_empl_nmfg sh_unempl sh_nilf sh_ssadiswkrs avg_lnwkwage_mfg avg_lnwkwage_nmfg trans_totindiv_pc trans_ssaret_pc trans_ssadis_pc trans_totmed_pc trans_fedinc_pc trans_unemp_pc trans_taaimp_pc avg_hhincsum_pc_pw avg_hhincwage_pc_pw {
   ...
   * this results in negative weights when l_popcount is noised negativelyl, bc d_popcount is not
   * we can skip it --- not a key result
...
*** EDITED by Ryan Steed
/* esttab using ../../results/table3.csv, b(%9.3f) se(%9.3f) nostar r2 drop(t* reg*) replace plain wide */
estout using ../../results/table3.csv, cells(`"b() se() t() p()"') stats(N r2 df_m df_r df_b df) notype abbrev begin(`"""') delimiter(`"",""') end(`"""') replace drop(t* reg*)
***
