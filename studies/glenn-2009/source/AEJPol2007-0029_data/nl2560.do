...
/* for 256 MB PC 100 memory modules                                           */
...
capture log close
log using nl2560.log, replace

use nldata2560.dta 

sort numdate h postal cnum
for num 1/12: gen byte bX=0
for num 1/12: replace bX=1 if cnum==2 & crank==X
for num 1/12: replace bX=1 if cnum[_n+1]==2 & crank[_n+1]==X & h==h[_n+1] & numdate==numdate[_n+1]

program define nlbase9
  version 7.0
  if "`1'"=="?" {
    global S_1 "HOME BOOST Taxwt PROX WKEND MINP PRICE SHIPTIME ST_FL ST_IL ST_OH ST_OR ST_PA ST_VA ST_WI ST_TX ST_AL ST_GA ST_AK ST_AZ ST_AR ST_CO ST_CT ST_DE ST_DC ST_HI ST_ID ST_IN ST_IA ST_KS ST_KY ST_LA ST_ME ST_MD ST_MA ST_MI ST_MN ST_MS ST_MO ST_MT ST_NE ST_NV ST_NH ST_NJ ST_NM ST_NY ST_NC ST_ND ST_OK ST_RI ST_SC ST_SD ST_TN ST_UT ST_VT ST_WA ST_WV ST_WY T1 T4 T7 T11"
