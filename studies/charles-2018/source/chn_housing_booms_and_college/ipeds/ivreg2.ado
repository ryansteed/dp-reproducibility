...
program define ivreg2, eclass byable(recall) /* properties(svyj) */ sortpreserve
	version 9.2
	local lversion 02.2.09
	local ranktestversion 01.1.03
...
* On replay, set flag so saved eqns aren't dropped
		...
		syntax [anything(name=0)] [if] [in] [aw fw pw iw/] [, /*
			...
			*/ B0(string) SMATRIX(string) WMATRIX(string) sw swpsd /*
...
**** Partial-out block ******************
		...
		if "`partial'" != "" {
			preserve
			local partial : subinstr local partial "_cons" "", all count(local partialcons)
...
* only when S matrix is calculated
				capture `ivreg2_cmd' `lhs' `cinexog1' /*
					...
					*/ `gmm2s' `bwopt' `kernopt' `dofmopt' `sw' `swpsd' /*
...
* id=underidentification statistic, wid=weak identification statistic
			...
			tempname ccf cdf cceval cdeval cd cc
...
* sdofminus used here so that F-stat matches test stat from regression with no partial
				if "`cluster'"=="" {
					scalar `widstat'=r(chi2)/r(N)*(`N'-`iv_ct'-`sdofminus'-`dofminus')/`exex1_ct'
				}
				else {
					scalar `widstat'=r(chi2)/(`N'-1) /*
					*/	*(`N'-`iv_ct'-`sdofminus') /*
...
* Reduced form needed for AR first-stage test stat.  Also estimated if requested.
				...
				doRF "`lhs'" "`inexog1'" "`exexog1'" /*
					...
					*/ `"`clopt'"' `"`bwopt'"' `"`kernopt'"' /*
					*/ `"`saverfprefix'"' /*
					*/ "`dofminus'" "`sdofminus'" `"`sw'"' `"`swpsd'"' "`ivreg2_cmd'"
...
* which works without partial because b0 is only endog regressors.

				if `inexog1_ct' + `cons' > 0 {
					qui `ivreg2_cmd' `lhs' `inexog' (=`exexog') `wtexp' if `touse', /*
						*/	`noconstant' dofminus(`dofminus') /*
						*/	`robust' `clopt' `bwopt' `kernopt' `sw' `swpsd' nocollin
				}
				else {
					tempname b1
...
* Should never reach here
		...
		if "`swpsd'"~="" {
			ereturn local hacsubtitleV "Stock-Watson psd heteroskedastic-robust statistics (BETA VERSION)"
		}
	}
...
di in smcl _c "{help `helpfile'##widtest:Weak identification test}"
		if "`e(vcetype)'`e(kernel)'"=="" {
di in gr _c " (Cragg-Donald Wald F statistic):"
		}
		else {
di in gr _c " (Kleibergen-Paap rk Wald F statistic):"
		}
di in ye _col(71) %8.3f e(widstat)
...
di in y %-12s "`vn'" _col(13) in gr "|" _col(17) in y %8.4f `sheapr2' _col(31) in gr "|" /*
			...
			*/ _col(54) in y %8.2f `pr2F' _col(67) %8.4f `pr2p'
		local i = `i' + 1
	}
di
	if "`robust'`cluster'" != "" {
...
di
	...
	tempname iddf idstat idp widstat idstatwald idstatwaldp
	scalar `iddf'=e(iddf)
	scalar `idstat'=e(idstat)
	scalar `idp'=e(idp)
	scalar `widstat'=e(widstat)
	if "`cluster'"== "" {
		scalar `idstatwald'=`widstat'*e(exexog_ct)/(e(N)-e(dofminus)-e(rankzz)-e(sdofminus))*(e(N)-e(dofminus))
	}
	else {
		scalar `idstatwald'=`widstat'*e(exexog_ct)*(e(N)-1)/(e(N)-e(rankzz)-e(sdofminus))*e(N_clust)/(e(N_clust)-1)
	}
	scalar `idstatwaldp'=chiprob(`iddf',`idstatwald')
di in smcl "{help `helpfile'##idtest:Underidentification tests}"
di in gr "Ho: matrix of reduced form coefficients has rank=K1-1 (underidentified)"
di in gr "Ha: matrix has rank=K1 (identified)"
	if "`robust'`kernel'"=="" {
di in ye "Anderson canon. corr. N*CCEV LM statistic" _c
	}
	else {
di in ye "Kleibergen-Paap rk LM statistic" _c
	}
di in gr _col(45) "Chi-sq(" in ye `iddf' in gr ")=" %-7.2f in ye `idstat' _col(64) in gr "P-val=" %6.4f in ye `idp'
	if "`robust'`kernel'"=="" {
di in ye "Cragg-Donald N*CDEV Wald statistic" _c
	}
	else {
di in ye "Kleibergen-Paap rk Wald statistic" _c
	}
di in gr _col(45) "Chi-sq(" in ye `iddf' in gr ")=" %-7.2f in ye `idstatwald' /*
	*/ _col(64) in gr "P-val=" %6.4f in ye `idstatwaldp'

di
di in smcl "{help `helpfile'##widtest:Weak identification test}"
di in gr "Ho: equation is weakly identified"
	if "`robust'`kernel'"=="" {
di in ye "Cragg-Donald Wald F-statistic" _c
	}
	else {
di in ye "Kleibergen-Paap Wald rk F statistic" _c
	}
di  _col(49) %8.2f `widstat'
...
program define doFirst, rclass
	...
	while "``i''" != "" {
		capture `ivreg2_cmd' ``i'' `inexog' `exexog' `weight' /*
				...
				*/ `dofmopt' `sdofmopt' `sw' `swpsd' small nocollin
		if _rc ~= 0 {
...
* Use regress since need only RSS and handles all cases, including perverse ones (e.g. no regressors)
			qui regress ``i'' `inexog' `weight' if `touse', `nocons'
			scalar `rssinc'=e(rss)
