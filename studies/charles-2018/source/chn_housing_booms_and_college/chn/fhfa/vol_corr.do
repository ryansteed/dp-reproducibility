...
twoway ///
 ...
 graph export ./corr_t2_vol.eps, replace
 !epstopdf ./corr_t2_vol.eps
 !sz ./corr_t2_vol.pdf
...
twoway (scatter iv2_vol iv2_log [aw = wgt], msize(small) mcolor(none) mlcolor(navy*.8) mlwidth(vthin) ) ///
   ...
   text(-0.12 0.12 "slope = 0.36 (0.09), R{superscript:2} = 0.16", place(e) size(small)) ///
   scheme(s2color) graphregion(fcolor(white)) legend(off) ///
   ...
   ytitle("Magnitude of Structural Break (Volume only)")

 graph export ./corr_vol.eps, replace
 !epstopdf ./corr_vol.eps
 !sz ./corr_vol.pdf
