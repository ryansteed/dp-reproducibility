...
df_Y<-read.csv("data\\data_for_Matrix_Completion.csv", header = TRUE)
df_treat <- read.csv("data\\treat_matrix_for_Matrix_Completion.csv", header = TRUE)
...
colnames(tADH)[1]<-"ADH"
library(foreign)
write.dta(data.frame(cbind(year,tmcpanel,telasticnet,telasticnetT,tDID,tADH)),"results\\mcpanel.dta")
