...
/* Below commented code defines McCrary's DCdensity program
qui {
//Notes:
...
void DCdensitysub(string scalar runvar, string scalar tousevar, real scalar c, real scalar b, ///
  ...
  real scalar loopover						//DL: Declaring loopover?
...
*Now that the program has been defined, we can test for balance in our forcing variable (non-modal win margin)

use "Gridlock main data for analysis.dta" //pull in dataset

	*Identify cities that expereince an election between a modal and non-modal candidate
	...
	*identification ONLY comes from first relevant election
		keep if datayear==earliest_relevantelection
	...
	*Generage non-modal win margin
		gen nm_margin=margin if nonmodal_wins==1		//Will want to have a non-modal margin of victory for RD graphs
		replace nm_margin=-margin if nonmodal_wins==0	//Non-modal margin of victory should be negative if non-modal lost.
	
	*Now call on McCary code
	DCdensity nm_margin, breakpoint(0) generate(Xj Yj r0 fhat se_fhat) graphname(fig2.pdf)
