* Do-file para generar la base que se utiliza para las Tablas XI, XII (Panel A.4), XIII (Column 4), A.6 (Panel B) y A.7 (Column 2)


** Generamos bases de datos que contienen la variable endogena X y el instrumento correspondiente bajo diferentes procedimientos 
*Base IV usando poblacion 2010
...
* Me quedo con los inmigrantes de los paises que quiero	(este procedimiento entiendo que no se hizo para la base anterior)
...
save FinalBase_permisos, replace
...
rename population populationbis
save FinalBase_permisos, replace


** Juntamos con base de poblacion
use population, clear
merge 1:m cod_com using FinalBase_permisos
drop if _merge==1
drop _merge
...
sort cod_com year
merge 1:1 cod_com year using FinalBase_permisos		// base que contiene las variables endogenas
keep if _merge==3
drop _merge
...
cd "$conf_intermed"
merge 1:1 cod_com using FinalBase_permisos		
drop _merge
save FinalBase_permisos, replace
...
save FinalBase101_permisos, replace
***************************************************
...
merge 1:1 cod_com using FinalBase101_permisos
keep if _merge==3
* Tiro todo lo que no vaya a usar como control o instrumento
drop _merge
save FinalBase101_permisos2pop2010, replace
********************************************************
}
...
drop if dup > 1
drop dup
collapse (mean) kish, by(region15 cod_com)
drop kish
...
foreach j in homicidios1 homicidios_chilenos1 homicidios_nchilenos1 homicidios2 homicidios_chilenos2 homicidios_nchilenos2{
	...
	*Se crean tres outcomes: Binaria (a) tome valor 1 si una comuna tenia crimen > 0 y ahora tiene crimen igual 0; Binaria (b) toma valor 1 si una comuna tenia 0 crimen y sigue teniendo 0; 	Binaria (c) toma valor 1 si una comuna tenia crimen  = 0 y ahora tiene crimen mayor 0
	//local name = substr("`var'", 1, 4)
	bysort cod_com : gen extmargin_`j'a = (`j'==0 & `j'[_n-1]>0)
	bysort cod_com : gen extmargin_`j'b = (`j'==0 & `j'[_n-1]==0)
	bysort cod_com : gen extmargin_`j'c = (`j'>0 & `j'[_n-1]==0)
}

keep if year==2017

** Genero outcome que toma un 1 si aumento el crimen entre 2017 y 2008 y 0 sino
foreach j in homicidios1 homicidios_chilenos1 homicidios_nchilenos1 homicidios2 homicidios_chilenos2 homicidios_nchilenos2 {
	gen intmargin_`j' = (d`j'pc>0)
}
...
merge 1:1 cod_com using FinalBase101_permisos2pop2010
keep if _merge==3
drop _merge


keep year cod_com region15 pop population hombre age share* delta* dlog* d* dl_* extmargin* intmargin*

cd "$conf_final"
save homicides_IV, replace
