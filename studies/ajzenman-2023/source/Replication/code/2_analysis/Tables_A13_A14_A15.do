*------------------------------------------------------------------------------*
* 						Tablas A.13, A.14, A.15 (Cols. 1-7)	  				   *
*------------------------------------------------------------------------------*

{

cd "$final"
use enusc_IV_share2002, clear
...
foreach outcome in del_problema3 del_afecta3 del_calvida3 del_inseg5 del_vict pc_19 del_tend_barrio del_tend_com del_tend_pais{
	...
	qui summ `outcome'
	restore
	outreg2 using "${Tables}/Table_A13.tex", `replace' tex(frag) pdec(3) dec(2) adec(2) stats(coef se pval) paren(se) bracket(pval) keep(deltaimm) ctitle(`: variable label `outcome'',IV) addstat(Mean DV, `r(mean)',  "\$ \widehat{\Delta migr_{mt}} \$", b_first, "\hspace{1cm}", se_b_first, F-stat, F, "Part. \$ R^2 \$", pRsq) nocons label  nonotes nor2  
...
foreach outcome in Index_Vivienda Index_Vecinos del_arma pc_20{
	...
	qui summ `outcome'
	restore
	outreg2 using "${Tables}/Table_A14.tex", `replace' tex(frag) pdec(3) dec(2) adec(2) stats(coef se pval) paren(se) bracket(pval) keep(deltaimm) ctitle(`: variable label `outcome'',IV) addstat(Mean DV, `r(mean)',  "\$ \widehat{\Delta migr_{mt}} \$", b_first, "\hspace{1cm}", se_b_first, F-stat, F, "Part. \$ R^2 \$", pRsq) nocons label  nonotes nor2  
	local replace "append" 
}
...
foreach outcome in vict_hurto vict_robosorpr vict_robovehi vict_roboviv vict_lesiones vict_roboviolent vict_agreg{
	...
	qui summ `outcome'
	restore
	outreg2 using "${Tables}/Table_A15.tex", `replace' tex(frag) pdec(3) dec(2) adec(2) stats(coef se pval) paren(se) bracket(pval) keep(deltaimm) ctitle(`: variable label `outcome'',IV) addstat(Mean DV, `r(mean)',  "\$ \widehat{\Delta migr_{mt}} \$", b_first, "\hspace{1cm}", se_b_first, F-stat, F, "Part. \$ R^2 \$", pRsq) nocons label  nonotes nor2  
	local replace "append" 
}
