...
areg log_homicidios1pc lnrate_stock_imm inter_2009-inter_2017 i.year age hombre d_miss i.year#c.bas_log_homicidios1pc, absorb(cod_com) vce(cluster cod_com)
...
twoway rcap ci1 ci2 time if time !=. , legend(off)  yscale(r(-2 0 2)) ylabel(-15 0 15, labsize(large)) xlabel(2008(1)2017, labsize(large)) || rscatter ci3 ci4 time if time !=. , msymbol(circle_hollow) msize(tiny) pstyle(p4) legend(off)  || scatter coef_es time if time !=., , yline(0, lstyle(foreground)) xtitle("Year", size(vlarge)) ytitle("Log Homicide Rate", size(vlarge))  graphregion(color(white)) ylab(,nogrid labsize(large)) legend(off) 
graph export "${Graphs}/Table_A6_homicide_rate.pdf" , as(pdf) replace
