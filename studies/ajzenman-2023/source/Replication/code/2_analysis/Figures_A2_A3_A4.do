...
*gen years=1+(year==2005)+2*(year==2006)+3*(year==2008)
gen years=(year==2005)+2*(year==2006)+3*(year==2008)

matrix table=J(1,2,.)
foreach var in vict_agreg del_problema3 del_calvida3 del_inseg5 del_vict del_arma del_tend_barrio del_tend_com del_tend_pais{
	matrix A = (1,.,.,.\2,.,.,.\3,.,.,.)
	matrix B = (1,.,.,.\2,.,.,.\3,.,.,.)
	matrix C = (1,.,.,.\2,.,.,.\3,.,.,.)
	reg `var' i.years##(`controls') i.years##(`shares') i.cod_com, r cluster(cod_com)
	
	test (_b[2.years#c.share_per]=0) (_b[3.years#c.share_per]=0)
	...
	*Los labels de las variables se crean cuando se genera la base en el dofile "Pretrends"
	qui twoway rcap ci1 ci2 time, legend(off) || scatter coef_es time, yline(0, lstyle(foreground)) xtitle("") ytitle("", size(vlarge))  graphregion(color(white)) ylab(#3 ,nogrid labsize(huge)) legend(off)  xlab(1 "2005" 2 "2006" 3 "2008", labsize(huge) angle(60)) title(`: variable label `var'', size(vhuge)) 
	//note( ///"F statistic = `Fb'" ///"P-value = `pvalb'", size(large))
	graph export "$Graphs/`var'_PERU.pdf" , as(pdf) replace
	
	*Grafico prom ponderado de los top5
	test (2.173*_b[2.years#c.share_per]+0.484*_b[2.years#c.share_bol]+0.092*_b[2.years#c.share_ecu]+0.087*_b[2.years#c.share_chi]+0.009*_b[2.years#c.share_bra]=0) (2.173*_b[3.years#c.share_per]+0.484*_b[3.years#c.share_bol]+0.092*_b[3.years#c.share_ecu]+0.087*_b[3.years#c.share_chi]+0.009*_b[3.years#c.share_bra]=0)
...
//		POR AHORA ESTO LO  HAGO A MANO, DESPUES LO SIGO Y SI SE PUEDE LO AUTOMATIZO PARA QUE NO HAYA QUE METER LOS SHARES A MANO
	...
	*Grafico prom ponderado de TODOS
	...
	graph export "$Graphs/`var'_all.pdf" , as(pdf) replace
}
