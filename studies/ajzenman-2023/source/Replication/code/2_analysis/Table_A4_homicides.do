...
mat iv_base =[J(1,10,.)\dl_homicidios1pc_iv_base\J(1,10,.)\ $part_r2_bas, ., $part_r2_NC, ., $part_r2_visa, ., $part_r2_perm, ., $part_r2_bas,.\ $IV_n_bas, ., $IV_n_NC, ., $IV_n_visa, ., $IV_n_perm, ., $IV_n_bas, .]


* COLUMNAS 2SLS PANEL

use IV_panel_homicides, clear
tab cod_com, gen(fe_comuna)
tab year, gen(fe_year)

qui ivreg2 log_homicidios1pc edad ismale fe_year* fe_comuna* (lnrate_stock_imm=imm_instr), robust cluster(cod_com) savefirst partial(fe_year* fe_comuna*)
...
qui ivreg2 log_homicidios1pc fe_year* fe_comuna* (lnrate_stock_imm=imm_instr), robust cluster(cod_com) savefirst partial(fe_year* fe_comuna*)
...
qui ivreg2 log_homicidios1pc edad ismale fe_year* fe_comuna* (lnrate_stock_imm_visa=imm_instr_visa), robust cluster(cod_com) savefirst partial(fe_year* fe_comuna*)
...
qui ivreg2 log_homicidios1pc edad ismale fe_year* fe_comuna* (lnrate_stock_imm_permiso=imm_instr_permiso), robust cluster(cod_com) savefirst partial(fe_year* fe_comuna*)
...
* Adao
* Denominador
qui ivreg2 log_homicidios1pc edad ismale fe_year1-fe_year9 fe_comuna1-fe_comuna100 (lnrate_stock_imm=imm_instr), robust cluster(cod_com) savefirst partial(fe_year1-fe_year9 fe_comuna1-fe_comuna100)
gen yvarminusendogvar = log_homicidios1pc-e(b)[1,colsof(e(b))]-e(b)[1,1]*lnrate_stock_imm
...
mat log_homicidios1pc_iv_panel=[J(1,10,.)\ $IV_PANEL_bas, $IV_PANEL_se_bas, $IV_PANEL_NC, $IV_PANEL_se_NC, $IV_PANEL_visa, $IV_PANEL_se_visa, $IV_PANEL_perm, $IV_PANEL_se_perm, $IV_PANEL_bas, $IV_se_PANEL_adao]
	
mat iv_panel = [J(1,10,.)\log_homicidios1pc_iv_panel\J(1,10,.)\ $part_r2_PANEL_bas, ., $part_r2_PANEL_NC, ., $part_r2_PANEL_visa, ., $part_r2_PANEL_perm, ., $part_r2_PANEL_bas, .\ $IV_PANEL_n_bas, ., $IV_PANEL_n_NC, ., $IV_PANEL_n_visa, ., $IV_PANEL_n_perm, ., $IV_PANEL_n_bas, .]

frmttable using "$Tables\Table_A4_homicides.tex", tex fr statmat(iv_base) substat(1) ctitles("", "Baseline", "No Controls", "Visas" , "Permits", "Adao" \ "", "", "", "", "", "") rtitles("Panel A: 2SLS in Differences"\""\ "Panel A.4: Log Homicide Rate" \"" \ "Log Imm Rate" \ "" \ ""  \ "First Stage Regression"\"Part. R-squared"\""\"N") replace sdec(2) a4
frmttable using "$Tables\Table_A4_homicides.tex", tex fr statmat(iv_panel) substat(1) rtitles("Panel B: 2SLS in Levels"\""\ "Panel B.4: Log Homicide Rate" \"" \ "Log Imm Rate" \ "" \ ""  \ "First Stage Regression"\"Part. R-squared"\""\"N") append replace sdec(2) a4
