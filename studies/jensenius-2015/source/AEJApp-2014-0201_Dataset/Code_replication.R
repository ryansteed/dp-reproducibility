...
row.names(mymatrix) <- c(
  "Percentage of SCs", "Literacy rate", " Employment Rate", "Agricultural laborers", "Electricity in village", "School in village ", "Medical facility in village", "Comm. channel in village",
...
row.names(mymatrix) <- c(
  "Percentage SCs", "Literacy rate ", "Employment rate ", "Agricultural laborers", "Electricity in village", "School in village ", "Medical facility in village", "Comm. channel in village",
  "Literacy gap", "Employment gap", "Agricultural laborers gap", "Electricity in village gap", "School in village gap", "Medical facility in village gap", "Comm. channel in village gap"
)
...
summary(model7lm)
summary(model9lm)
model_summary_1 <- summary(model3lm)
summary_df_1 <- as.data.frame(model_summary_1$coefficients)

# Write to CSV
write.csv(summary_df_1,
  file = "../../results/model3lm_summary.csv",
  row.names = TRUE
)

model_summary_2 <- summary(model6lm)
summary_df_2 <- as.data.frame(model_summary_2$coefficients)

# Write to CSV
write.csv(summary_df_2,
  file = "../../results/model6lm.csv",
  row.names = TRUE
)

model_summary_3 <- summary(model9lm)
summary_df_3 <- as.data.frame(model_summary_3$coefficients)

# Write to CSV
write.csv(summary_df_3,
  file = "../../results/model9lm.csv",
  row.names = TRUE
)
