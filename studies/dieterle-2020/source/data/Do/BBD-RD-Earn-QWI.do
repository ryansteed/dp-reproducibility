* Generate Estimates for Table 2 Row 2
...
foreach y of varlist ln_earn ln_ui{
foreach r in 1 aicc{
...
forvalues t=0/1{
capture: sum ln_earn if treat==`t' & st_bound==`i' & qtr==`j'
if r(N)>0{
clear results
capture noisily: reg ln_earn [aweight=wgt]  if  treat==`t' &    st_bound==`i' & qtr==`j'
...
forvalues r=1/6{
clear results
capture noisily: reg ln_earn mu1-mu`r' [aweight=wgt]  if  treat==`t' &    st_bound==`i' & qtr==`j'
...
quietly: sum ln_earn if spec_aicc_0==`s0' & spec_aicc_1==`s1' & st_bound==`i'
if r(N)>0{
...
areg ln_earn ln_ui [aweight=wgt]  if     ln_earn_res_1<., absorb(st_bound_qtr_id) cluster(st_by_bound)

*Columns (2) and (3)
foreach r in 1 aicc{
areg ln_earn_res_`r' ln_ui_res_`r' [aweight=wgt] , absorb(st_bound_qtr_id) cluster(st_by_bound)
}


/*Confirm point estimate from Frisch-Waugh for linear case
areg ln_earn ln_ui c.mu1#i.st_bound_qtr_id#i.treat [aweight=wgt]  if     ln_earn_res_1<., absorb(st_bound_qtr_id) cluster(st_by_bound)
*/
