* Generate Estimates for Table C.1 row (1)
...
foreach y of varlist ln_unemp treat_pran{
foreach r in 1 aicc{
...
forvalues t=0/1{
capture: sum ln_unemp if treat_pran==`t' & st_bound==`i' & qtr==`j'
if r(N)>0{
clear results
capture noisily: reg ln_unemp [aweight=wgt]  if  treat_pran==`t' &    st_bound==`i' & qtr==`j'
...
forvalues r=1/6{
clear results
capture noisily: reg ln_unemp mu1-mu`r' [aweight=wgt]  if  treat_pran==`t' &    st_bound==`i' & qtr==`j'
...
forvalues i=`s_min'/`s_max'{
di "State Boundary `i'"
capture: sum spec_aicc_0 if treat_pran==`t' & st_bound==`i'
local s=r(mean)
clear results
...
areg ln_unemp treat_pran [aweight=wgt]  if     ln_unemp_res_1<., absorb(st_bound_qtr_id) cluster(st_by_bound)
nlcom exp(ln(.05)+(_b[treat_pran]))


*Columns (2) and (3)
foreach r in 1 aicc{
areg ln_unemp_res_`r' treat_pran_res_`r' [aweight=wgt] , absorb(st_bound_qtr_id) cluster(st_by_bound)
nlcom exp(ln(.05)+(_b[treat_pran_res_`r']))
}


/*Confirm point estimate from Frisch-Waugh for linear case
areg ln_unemp treat_pran c.mu1#i.st_bound_qtr_id#i.treat_pran [aweight=wgt]  if     ln_unemp_res_1<., absorb(st_bound_qtr_id) cluster(st_by_bound)
*/
