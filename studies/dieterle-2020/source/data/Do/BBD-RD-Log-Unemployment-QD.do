* Generate Estimates for Table 3 columns (2), (3), and (4)
...
foreach y of varlist ln_unemp_qd ln_ui{
foreach r in 1 aicc{
...
forvalues t=0/1{
capture: sum ln_unemp_qd if treat==`t' & st_bound==`i' & qtr==`j'
if r(N)>0{
clear results
capture noisily: reg ln_unemp_qd [aweight=wgt]  if  treat==`t' &    st_bound==`i' & qtr==`j'
...
forvalues r=1/6{
clear results
capture noisily: reg ln_unemp_qd mu1-mu`r' [aweight=wgt]  if  treat==`t' &    st_bound==`i' & qtr==`j'
...
quietly: sum ln_unemp_qd if spec_aicc_0==`s0' & spec_aicc_1==`s1' & st_bound==`i'
if r(N)>0{
...
*Column (2): No distance control
areg ln_unemp_qd ln_ui [aweight=wgt]  if     ln_unemp_qd_res_1<., absorb(st_bound_qtr_id) cluster(st_by_bound)
nlcom exp(ln(.05)+(_b[ln_ui])*((1-(.891)^16)/(1-(.891)))*(ln(82.5)-ln(26)))
nlcom exp(ln(.05)+(_b[ln_ui])*((1)/(1-(.891)))*(ln(99)-ln(26)))

*Columns (3) and (4)
foreach r in 1 aicc{
areg ln_unemp_qd_res_`r' ln_ui_res_`r' [aweight=wgt] , absorb(st_bound_qtr_id) cluster(st_by_bound)
nlcom exp(ln(.05)+(_b[ln_ui])*((1-(.891)^16)/(1-(.891)))*(ln(82.5)-ln(26)))
nlcom exp(ln(.05)+(_b[ln_ui])*((1)/(1-(.891)))*(ln(99)-ln(26)))
}
...
areg ln_unemp_qd ln_ui c.mu1#i.st_bound_qtr_id#i.treat [aweight=wgt]  if     ln_unemp_res_1<., absorb(st_bound_qtr_id) cluster(st_by_bound)
*/
